...
class Events:
    ...
    async def on_reaction_add(self, reaction, user):
        if reaction.message.channel.id == 445658065933434892 and (reaction.message.attachments or reaction.message.embed) and not user.bot and user.id != reaction.message.author.id and reaction.emoji.id == 507362047059689472:
    ...
    async def on_reaction_remove(self, reaction, user):
        if reaction.message.channel.id == 445658065933434892 and (reaction.message.attachments or rection.message.embed) and not user.bot and user.id != reaction.message.author.id and reaction.emoji.id == 507362047059689472:
            query = "SELECT * FROM artstats WHERE userid = $1;"
            row = await self.bot.db.fetchrow(query, reaction.message.author.id)
            if row is None:
